...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        .addFields({ name: 'Dislike Sayısı:', value: `${queue.songs[0].dislikes}`, inline: true })
            .addFields({ name: 'Filtre:', value: `${queue.filters.names.join(', ') || "Normal"}`, inline: true })
            .addFields({ name: `Geçerli Süre: \`[${queue.formattedCurrentTime} / ${queue.songs[0].formattedDuration}]\``, value: `\`\`\`${uni} ${'─'.repeat(part) + '🎶' + '─'.repeat(30 - part)}\`\`\``, inline: true })
            .setTimestamp()
